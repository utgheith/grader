...
case class Project(course: Course, project_name: String)
  ...
  lazy val prepare
      : (CSID, CutoffTime) => Target[WithData[Option[PrepareInfo]]] = fun {
    (csid, cutoff) =>
      target(
        submission(csid),
        has_student_report(
  ...
  private lazy val publish_student_results
      : (CSID, Int) => Target[WithData[StudentResults]] = fun { (csid, n) =>
    complex_target {
      val prepared = prepare(csid, CutoffTime.None).track
      val repo_info =
  ...
  lazy val publish_results
      : Int => Target[WithData[SortedMap[Alias, RedactedStudentResults]]] =
    fun { n =>
      complex_target {
  ...
  private lazy val compute_student_scores
      : (CSID, CutoffTime, Int) => Target[SortedSet[TestId]] = fun {
    (csid, cutoff_time, n) =>
      complex_target {
        val outs = for {
  ...
  lazy val compute_scores
      : (CutoffTime, Int) => Target[SortedMap[CSID, SortedSet[TestId]]] = fun {
    (cutoff_time, n) =>
      complex_target {
        val pairs = for {
          csids <- students_with_submission.track
          pairs <- Future.sequence {
            csids.toSeq.map { csid =>
              compute_student_scores(csid, cutoff_time, n).track.map(g =>
                (csid, g)
              )
            }
          }
