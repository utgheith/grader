...
case class Project(course: Course, project_name: String) extends Scope(course / project_name) derives ReadWriter {
  ...
  lazy val publish_results: Int => Target[WithData[SortedMap[Alias, RedactedStudentResults]]] = fun { n =>
    complex_target {
  ...
  lazy val chosen_test_ids: Target[SortedSet[TestId]] =
    target(test_ids, chosen) { (tests, chosen) =>
      for {
        test <- tests
  ...
  private lazy val compute_student_scores: (CSID, CutoffTime, Int) => Target[SortedSet[TestId]] = fun { (csid, cutoff_time, n) =>
    complex_target {
      val outs = for {
  ...
  lazy val compute_scores: (CutoffTime, Int) => Target[SortedMap[CSID, SortedSet[TestId]]] = fun { (cutoff_time, n) =>
    complex_target {
      val pairs = for {
        csids <- students_with_submission.track
        pairs <- Future.sequence {
          csids.toSeq.map { csid =>
            compute_student_scores(csid, cutoff_time, n).track.map(g => (csid, g))
          }
        }
      } yield pairs
