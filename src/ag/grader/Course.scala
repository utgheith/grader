...
import ag.rules.{Maker, Periodic, Rule, SignedPath, given_ReadWriter_SortedMap, run, say}
...
case class NotificationConfig(
  ...
  private def doit(to: CSID, cc: Option[CSID], subject: => String, contents: => String): Unit = {
  ...
  private def send(csid: CSID, subject: => String, contents: => String): Unit = {
  ...
  def send_key_update(course: Course, csid: CSID, server: RemoteServer): Unit = if (key_update) {
    send(
      csid,
    ...
    say(s"    key update notification disabled for ${course.course_name}::$csid")
  }

  def send_repo_created(project: Project, repo_info: RepoInfo, csid: CSID): Unit= if (repo_create) {
    val msg = s"git clone ${repo_info.server.git_uri(project.work_repo_name(csid))}"
    send(
      csid,
      msg,
      s"""
    ...
    say(s"    repo creation notification is disabled for ${project.course.course_name}::${project.project_name}::$csid")

  }

  def send_result_update(project: Project, csid: CSID, server: RemoteServer, student_results: RedactedStudentResults): Unit = if (result_update) {
    ...
    val n_pass = student_results.outcomes.values.filter(_.outcome.contains("pass")).size
    ...
    val subject = s"[$n_pass/$n_tests:${has_test}T:${has_report}R] ${course_name}_${project_name}_${csid} [$short_sha]"

    val not_passing = (for {
      (_, outcome) <- student_results.outcomes
      if ! outcome.outcome.contains("pass")
    } yield s"${outcome.test_id.external_name} ... ${outcome.outcome.getOrElse("?")}").mkString("\n")
    ...
    val contents = s"""
      ...
      |your details: git clone ${server.git_uri(project.student_results_repo_name(csid))}
      |all tests: git clone ${server.git_uri(project.tests_repo_name)}
      |summary for project: git clone ${server.git_uri(project.project_results_repo_name)}
      |
      |$not_passing
      """.stripMargin
    ...
    say(s"    result update notification is disabled for ${project.course.course_name}::${project.project_name}::${csid}")
  }
}
...
case class RawCourseNotSorted(
  ...
  lazy val sorted: RawCourse = RawCourse(active=active, notifications=notifications, projects=projects.to(SortedMap))
}
...
case class Course(course_name: String) derives ReadWriter {
  ...
  lazy val notifications: Maker[NotificationConfig] = Rule(raw, scope)(_.notifications)
  ...
  lazy val active_projects: Maker[SortedMap[String, Project]] =
    Rule(Maker.select(projects.map(_.values.toSeq))(_.active), scope) { projects =>
  ...
  /* students who dropped keys in the dropbox.
   * This is a list of potentially enrolled students but enrollment is only
   * completed when the key is added to gitolite-admin
   * 
   * Can only run on a CS machine.
   */
  /*
  lazy val _dropbox: Maker[SortedMap[CSID, StudentInfo]] = Rule(Periodic(ms=60*1000) *: Config.cs_server *: Gitolite.mirror("gitolite-admin"), scope) { case (_, cs_server, _) =>
    val (_, stdout_path, _) = cs_server.ssh("python3", "get_keys.py", course_name).run(cwd = os.pwd, check=true)


    val m = upickle.default.read[Map[CSID, StudentInfo]](os.read(stdout_path))

    m.foreach {
      case (_, si) =>
        if (si.owner != si.csid) {
          throw Exception(s"owner [${si.owner}] != csid [${si.csid}")
        }
    }
  ...
  lazy val dropbox: Maker[SortedMap[CSID, String]] = Rule(Periodic(60000) *: Config.dropbox_path, scope) {
    case (_, Some(dropbox)) =>
    ...
    SignedPath.rule(
      ...
      val added = g.update(
        ...
        for {
          ...
          if !os.exists(key_file_path) || os.read(key_file_path).trim.nn != new_key
        } {
  ...
  lazy val enrollment: Maker[SortedMap[CSID, String]] =
    Rule(Gitolite.mirror(enrollment_repo_name), scope) { sp =>
      if (sp.data) {
        ...
        if (os.isFile(f)) {
          upickle.default.read[SortedMap[CSID, String]](os.read(enrollment_file(sp.path)))
        } else {
  ...
  lazy val publish_enrollment: Maker[SignedPath[SortedMap[CSID, String]]] =
    SignedPath.rule(
      ...
      g.update(
        ...
        os.write.over(enrollment_file(dir), upickle.default.write(keys.data, indent=2))
        keys.data
      }
    }
