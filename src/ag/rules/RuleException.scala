...
object RuleException {
  def apply(r: RuleBase, cause: Throwable): RuleException = cause match {
    case RuleException(thread_name, chain, cause) => new RuleException(thread_name, r +: chain, cause)
    case NonFatal(e)                 => new RuleException(Thread.currentThread.getName, Seq(r), e)
  }
}
