...
case class SignedPath[D](path: os.Path, signature: Signature, ignore_last: SortedSet[String], data: D) derives ReadWriter {
}
...
object SignedPath {
  ...
  def rule[In, D: ReadWriter](in: Maker[In], ignore_last: SortedSet[String], rest: os.RelPath|Null)(f: (os.Path, In) => D)(using
    ...
    override def compute(context: Context, in: In): SignedPath[D] = {
      ...
      os.remove.all(dirty_file)
      SignedPath(base_dir, Signature.of(SortedSet(base_dir), ignore_last), ignore_last, d)
    }
  }
}
